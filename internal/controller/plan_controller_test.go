...
// (both already Ready, with their required Secrets present) so a
...
func planTestFixtures(gatewayAddr string) (*billingv1alpha1.InvoraBillingInstance, *billingv1alpha1.InvoraBillingOrganization, *corev1.Secret, *corev1.Secret) {
	...
	orgSecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "org-secret", Namespace: "default"},
		Data:       map[string][]byte{"apiKey": []byte("test-org-api-key")},
	}
	...
	return instance, org, instSecret, orgSecret
}
...
func TestPlanReconciler_AdoptsExistingPlanByCode(t *testing.T) {
	...
	instance, org, instSecret, orgSecret := planTestFixtures(addr)
	...
	r := &InvoraBillingPlanReconciler{BaseReconciler: BaseReconciler{
		Client: fake.NewClientBuilder().
			WithScheme(s).
			WithObjects(instance, org, instSecret, orgSecret, plan).
